...
from ai.prompt_builder import build_sql_correction_prompt, build_sql_prompt
        ...
        try:
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    result["sql"] = (
                        raw_sql  # Update with the successful one if it changed
                    )
                    break
                    ...
                    if attempt < max_retries - 1:
                        ...
                        correction_prompt = build_sql_correction_prompt(
                            user_question, raw_sql, str(e)
                        )
                        raw_sql = generate_completion(
            ...
            result["explanation"] = generate_insight(
                user_question, raw_sql, data_summary
            )
