...
def validate_sql(sql: str) -> str:
    ...
    if ";" in sql_clean and sql_clean.rfind(";") != len(sql_clean) - 1:
        raise UnsafeSQLError("Multiple SQL statements (semicolon chaining) are not allowed.")
    
    # 2. Block DML and DDL
    forbidden_keywords = [
        "INSERT", "UPDATE", "DELETE", "DROP", "ALTER", "TRUNCATE", 
        "CREATE", "GRANT", "REVOKE", "COPY", "VACUUM", "REPLACE"
    ]
    upper_sql = sql_clean.upper()
    for kw in forbidden_keywords:
        if re.search(r'\b' + kw + r'\b', upper_sql):
            raise UnsafeSQLError(f"Destructive or modifying operations ({kw}) are strictly prohibited.")
