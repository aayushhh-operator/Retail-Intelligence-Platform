...
from airflow.plugins.operators.data_generation_operator import DataGenerationOperator
...
from airflow.plugins.operators.transformation_operator import TransformationOperator
...
from airflow.plugins.utils.airflow_logger import (
    log_dag_run_start, log_dag_run_success, log_dag_run_failure,
    log_task_start, log_task_success, log_task_failure
)
...
with DAG(
    ...
    start = EmptyOperator(
        task_id='start',
        on_execute_callback=log_dag_run_start
    )
    
    with TaskGroup('data_generation', tooltip='Generate Synthetic Data') as data_generation_group:
    ...
    # Dependencies
    start >> data_generation_group >> extract_data >> validate_data >> transform_data >> load_warehouse >> build_analytics >> end
