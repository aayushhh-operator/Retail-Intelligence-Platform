...
if not prod_df.empty:
    ...
    fig = px.bar(prod_df, x="revenue", y="product_name", orientation='h', title="Revenue by Product")
    st.plotly_chart(fig, use_container_width=True)
    
    st.dataframe(prod_df.sort_values(by="revenue", ascending=False), use_container_width=True)
else:
