...
st.markdown(
    "Ask natural language questions about your retail platform. I will generate SQL, query the warehouse, and visualize the results!"
)
...
for idx, msg in enumerate(st.session_state.messages):
    with st.chat_message(msg["role"]):
        ...
        if "chart" in msg and msg["chart"] is not None:
            st.plotly_chart(
                msg["chart"], use_container_width=True, key=f"chart_history_{idx}"
            )
            ...
            if response["error"]:
                ...
                if response["chart"]:
                    ...
                    st.plotly_chart(
                        ...
                        key=f"chart_new_{uuid.uuid4().hex}",
                    )
