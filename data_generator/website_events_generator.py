...
def generate_website_events(rng: random.Random) -> list[dict[str, Any]]:
    ...
    event_types = ["page_view", "add_to_cart", "remove_from_cart", "checkout_started", "purchase", "search"]
    ...
    for i in range(1, config.NUMBER_OF_WEBSITE_EVENTS + 1):
        ...
        event_date = fake.date_time_between(start_date=config.START_DATE, end_date=config.END_DATE)
        ...
        events.append(
            {
                ...
                "customer_id": make_id("CUS", rng.randint(1, config.NUMBER_OF_CUSTOMERS), width=6) if rng.random() > 0.3 else None,
                "url": fake.uri(),
