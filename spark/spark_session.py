...
from spark.config import (CHECKPOINT_DIR, SPARK_APP_NAME, SPARK_DRIVER_MEMORY,
                          SPARK_EXECUTOR_MEMORY, SPARK_MASTER)
...
class SparkManager:
    ...
    def get_session(cls) -> SparkSession:
        ...
        if cls._spark is None:
            logger.info(
                f"Initializing Spark Session: {SPARK_APP_NAME} on {SPARK_MASTER}"
            )
            try:
                cls._spark = (
                    ...
                    )  # Reduced for local/small datasets
