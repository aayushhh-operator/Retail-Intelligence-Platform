...
def sessionize_events(events_df: DataFrame, timeout_minutes: int = 30) -> DataFrame:
    ...
    session_window = Window.partitionBy("customer_id").orderBy("event_timestamp").rowsBetween(Window.unboundedPreceding, 0)
    df = df.withColumn("session_group_id", F.sum("is_new_session").over(session_window))
    
    # Create final unique session ID
    df = df.withColumn("session_id", F.concat_ws("_", F.col("customer_id"), F.col("session_group_id")))
    
    return df.drop("prev_timestamp", "time_diff_seconds", "is_new_session", "session_group_id")
