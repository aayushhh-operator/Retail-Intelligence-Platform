...
from validate.utils import (EMAIL_PATTERN, PHONE_PATTERN, ZIP_PATTERN,
                            is_blank, parse_date, to_float, today)
...
class BusinessRuleValidator:
    ...
    def _generic_rules(
        ...
        blank_rows = sum(
            1 for row in rows if any(is_blank(row.get(column)) for column in columns)
        )
        ...
        return [
            RuleResult(
                ...
                "WARNING",
                blank_rows,
                f"{blank_rows} rows contain blank values",
            ),
            RuleResult(
                "duplicate_rows",
                duplicate_rows == 0,
    ...
    def _validate_customers(
        self, rows: list[dict[str, Any]], _context: dict[str, set[str]]
    ) -> list[RuleResult]:
        return [
    ...
    def _validate_orders(
        self, rows: list[dict[str, Any]], context: dict[str, set[str]]
    ) -> list[RuleResult]:
        return [
            _unique_key(rows, "order_id"),
            _foreign_key(
                rows, "customer_id", context.get("customers", set()), "customer_exists"
            ),
            _foreign_key(
                rows, "product_id", context.get("products", set()), "product_exists"
            ),
    ...
    def _validate_payments(
        ...
        return [
            ...
            _foreign_key(
                rows, "order_id", context.get("orders", set()), "order_exists"
            ),
            _positive_number(rows, "amount"),
            _allowed_values(
                rows, "payment_method", ALLOWED_PAYMENT_METHODS, "valid_payment_method"
            ),
    ...
    def _validate_shipping(
        ...
        return [
            ...
            _date_order_rule(
                rows, "dispatch_date", "delivery_date", "delivery_after_dispatch"
            ),
        ]
...
def _not_blank(rows: list[dict[str, Any]], column: str) -> RuleResult:
    ...
    return RuleResult(
        ...
        "ERROR",
        count,
        f"{count} blank {column} values",
        [column],
    )


def _regex_rule(
    rows: list[dict[str, Any]], column: str, pattern: Any, rule_name: str
) -> RuleResult:
    count = sum(
        1
        for row in rows
        if not is_blank(row.get(column)) and not pattern.match(str(row.get(column)))
    )
    return RuleResult(
        rule_name,
        count == 0,
...
def _positive_number(rows: list[dict[str, Any]], column: str) -> RuleResult:
    count = sum(
        1 for row in rows if (value := to_float(row.get(column))) is None or value <= 0
    )
    return RuleResult(
        f"positive_{column}",
        count == 0,
        "ERROR",
        count,
        f"{count} non-positive {column} values",
        [column],
    )
...
def _non_negative_number(rows: list[dict[str, Any]], column: str) -> RuleResult:
    ...
    return RuleResult(
        ...
        "ERROR",
        count,
        f"{count} negative {column} values",
        [column],
    )
...
def _number_between(
    ...
    count = sum(
        1
        for row in rows
        if (value := to_float(row.get(column))) is None
        or value < minimum
        or value > maximum
    )
    return RuleResult(
        ...
        "ERROR",
        count,
        f"{count} {column} values outside {minimum}-{maximum}",
        [column],
    )
...
def _date_not_future(rows: list[dict[str, Any]], column: str) -> RuleResult:
    ...
    count = sum(
        1
        for row in rows
        if (value := parse_date(row.get(column))) is None or value > current_date
    )
    return RuleResult(
        ...
        "ERROR",
        count,
        f"{count} invalid or future {column} values",
        [column],
    )


def _allowed_values(
    rows: list[dict[str, Any]], column: str, allowed: set[str], rule_name: str
) -> RuleResult:
    count = sum(
        1
        for row in rows
        if not is_blank(row.get(column)) and str(row.get(column)) not in allowed
    )
    return RuleResult(
        rule_name,
        count == 0,
        "WARNING",
        count,
        f"{count} unexpected {column} values",
        [column],
    )


def _foreign_key(
    rows: list[dict[str, Any]], column: str, valid_values: set[str], rule_name: str
) -> RuleResult:
    count = sum(
        1
        for row in rows
        if is_blank(row.get(column)) or str(row.get(column)) not in valid_values
    )
    return RuleResult(
        rule_name,
        count == 0,
        "ERROR",
        count,
        f"{count} {column} values do not reference known records",
        [column],
    )


def _date_order_rule(
    rows: list[dict[str, Any]], start_column: str, end_column: str, rule_name: str
) -> RuleResult:
    count = 0
    for row in rows:
    ...
    return RuleResult(
        ...
        "ERROR",
        count,
        f"{count} rows violate date order",
        [start_column, end_column],
    )


def _review_after_purchase(
    rows: list[dict[str, Any]], context: dict[str, set[str]]
) -> RuleResult:
    ...
    for row in rows:
        ...
        if (
            pair not in delivered_pairs
            or review_date is None
            or review_date > today() + timedelta(days=366)
        ):
            count += 1
    return RuleResult(
        "verified_purchase_logic",
        count == 0,
        "ERROR",
        count,
        f"{count} reviews do not align to delivered purchases",
        ["customer_id", "product_id", "review_date"],
    )


def _outlier_rule(
    rows: list[dict[str, Any]], column: str, rule_name: str | None = None
) -> RuleResult:
    ...
    if len(values) < 4:
        return RuleResult(
            ...
            "WARNING",
            0,
            f"Insufficient data for outlier detection on {column}",
            [column],
        )
    ...
    outlier_count = sum(
        1
        for _, v in numeric_pairs
        if v is not None and (v < lower_fence or v > upper_fence)
    )
    return RuleResult(
        name,
        outlier_count == 0,
