...
from warehouse.config import DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD
...
class DatabaseConnection:
    ...
    def connect(self) -> None:
        ...
        try:
            ...
            raise DatabaseConnectionError(f"Failed to connect to {DB_NAME} at {DB_HOST}:{DB_PORT}") from e

    def get_session(self) -> Session:
        """Get a new SQLAlchemy session."""
        if not self.SessionLocal:
            raise DatabaseConnectionError("Engine not initialized. Call connect() first.")
