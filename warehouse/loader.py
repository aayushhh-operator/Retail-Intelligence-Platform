...
from warehouse.exceptions import SchemaValidationError, LoadError
import logging
...
class WarehouseLoader:
    ...
    def load_dataset(self, filepath: Path, dataset_name: str, schema: str = "warehouse", strategy: str = "Replace") -> int:
        ...
        try:
            ...
            with self.transaction_mgr.transaction() as session:
                if strategy.lower() == "replace":
                    ...
                    session.execute(text(f"TRUNCATE TABLE {schema}.{dataset_name} CASCADE;"))
                
                rows_loaded = self.csv_loader.load(filepath, dataset_name, schema, session)
